...
import com.emt.bpay.rapi.RemoteApiConfig;
...
public class PayCurrentController {
    ...
    public ReturnObject DocTypeGroupReport(String paramJson) throws JSONException {
        ...
        try
        {
            ...
            logger.info("DocTypeGroupReport传入参数：{}", _json);
        ...
        jsonObject.put("bDate", bDate);
        jsonObject.put("eDate", eDate);


        List<String> msg = new ArrayList<String>();
        List<Map<String, Object>> list = payCurrentSv.DocTypeGroupReport(jsonObject.toString());
    ...
    public PageInfo PageCurrent(int pageIndex, int pageSize, String accountJson){
        ...
        if (StrUtil.isNotEmpty(accountJson))
        {
            try
            {
                ...
                if (StringUtils.isNotEmpty(vo.getB_reg_time())){
                    vo.setB_reg_time(vo.getB_reg_time()+" 0:0:0");
                }
                if (StringUtils.isNotEmpty(vo.getE_reg_time())){
                    vo.setE_reg_time(vo.getE_reg_time() + " 23:59:59");
                }
                vo.setSys_id(RemoteApiConfig.sysId);
        ...
        PageInfo page = payCurrentSv.CurrentByPage(pageIndex, pageSize, vo);
        logger.info("CurrentByPage分页返回数据：{}", page);
        return page;
    }
    ...
    public   List<Map<String, Object>> ExportCurrent(String paramJson){
        ...
        String payeeId = "";
        String userId="";
        String sysId= RemoteApiConfig.sysId;
        String token="";
        try
        {
        ...
        try {
            ...
            vo.setB_reg_time(bDate);
            vo.setE_reg_time(eDate);
            return   payCurrentSv.CurrentToMap(vo);
